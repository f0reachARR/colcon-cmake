...
class CmakeBuildTask(TaskExtensionPoint):
    ...
    async def build(  # noqa: D102
        ...
        *,
        ...
        logger.info("Building CMake package in '{args.path}'".format_map(locals()))
        ...
        if project_name is None:
            ...
            logger.warning(
                ...
                "CMake cache has no 'CMAKE_PROJECT_NAME' variable".format_map(locals())
            )
            return
        ...
        if not args.cmake_target:
            ...
            if args.cmake_clean_after_build:
                self.progress("clean")
    ...
    async def _reconfigure(self, args, env):
        ...
        if os.name == "nt" and generator is None:
            ...
            if vsv not in supported_vsv:
                raise RuntimeError(
                    "Unknown / unsupported VS version '{vsv}'".format_map(locals())
                )
        ...
        completed = await run(
            self.context, [CMAKE_EXECUTABLE] + cmake_args, cwd=args.build_base, env=env
        )
    ...
    def _get_last_cmake_args(self, build_base):
        ...
        try:
            ...
            logger.error(
                "Failed to parse previous --cmake-args from '{path}': {e}".format_map(
                    locals()
                )
            )
            return None
